...
window.resizable(width=False, height=False)     # Block Window Size
window.config(bg="#3b3b3b")                     # black background
window.title("Tic Tac Toe")                     # Name of the window

# Grab image from URL to tkinter window icon
url = "https://i.imgur.com/yWrLiLt.png"
with urllib.request.urlopen(url) as connection:
    raw_data = connection.read()
icon_image = Image.open(io.BytesIO(raw_data))
image_url = ImageTk.PhotoImage(icon_image)

window.iconwindow(True, image_url)

players = ["X", "O"]                            # Players 
player = random.choice(players)                 # First Moves random...
buttons = [[0, 0, 0],                           # 2D List (TABLE)
           [0, 0, 0], 
           [0, 0, 0]]                           
...
label.pack(side="top")
# restart button 
restart_button = Button(text="Restart", font=("Consolas", 20), command=new_game, bg="#3b3b3b", fg="#ffffff")
